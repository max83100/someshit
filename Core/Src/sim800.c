...
UART_HandleTypeDef *sim800_uart = &huart1;
...
void itoa(int num, char *str, int radix)
{
    ...
    * Radix MUST be > 0 && <= 36
    */
    assert( radix > 0 && radix <= 36 );
    
    count = 0;
    if ((sign = num) < 0)
    num = -num;
    do {
        str[count++] = digits[num % radix];
    } while ((num /= radix) > 0);
...
static void cmgs_handler(void *handle_ptr, uint32_t msg_index);
...
SIM800_Status_t SIM800_SetSavingModeMode(SIM800_Handle_t *handle)
{
    char cmd[] = "AT+CPMS=\"SM\",\"SM\",\"SM\"\r\n";
    uint8_t index = add_pending_message(handle, "+CPMS", NULL);
...
SIM800_Status_t SIM800_SetLowSMSIndication (SIM800_Handle_t *handle)
{
        char cmd[] = "AT+CNMI=2,1\r\n";
    uint8_t index = add_pending_message(handle, "+CNMI", NULL);
...
SIM800_Status_t SIM800_SendSMSMessage(SIM800_Handle_t *handle, char *destination, char *message)
{
    ...
    uint8_t index = add_pending_message(handle, "+CMGS", cmgs_handler);
...
uint8_t SIM800_SetWaitingMess(SIM800_Handle_t *handle)
{
    return add_pending_message(handle, "+CMGS", cmgs_handler);
}
void SIM800_RemoveWaitingMess(SIM800_Handle_t *handle, uint8_t theIndex)
{
    remove_expected_code(handle, theIndex);
    myPDUSucc = 0;
}
...
uint8_t SIM800_PDUSuccess(void)
{
    return myPDUSucc; 
}

inline void SIM800_Init()
{
...
void SIM800_Config()
{
  ControllerIndication_DebugInfo("Display initialized");
    
  for(uint8_t anInd = 0; anInd < 5; ++anInd)
  {
    HAL_UART_Transmit(&huart1,(uint8_t *)"AT\r\n",strlen("AT\r\n"),1000);
    HAL_Delay(200);
  }
  
  HAL_UART_Transmit(&huart1,(uint8_t *)"ATZ+IPR=9600\r\n",strlen("ATZ+IPR=9600\r\n"),1000);
  HAL_Delay(1000);
  ...
  if(SIM800_SetSavingModeMode(&sim800h) != SIM800_OK)
  {
      Error_Handler();
  }
  
  if(SIM800_SetLowSMSIndication(&sim800h) != SIM800_OK)
  {
      Error_Handler();
  }
  
  ControllerIndication_DebugInfo("Set SMS text mode");
...
inline SIM800_Commands_t* GetSIMCommands()
{
    return SIM800_Commands;
}

inline uint8_t GetSIMCommandsCount()
{
    return myCommandsCount;
}

inline void ClearSIMCommands()
{
    memset(SIM800_Commands, 0, sizeof(SIM800_Commands));
    myCommandsCount = 0;
}
...
static bool addCommand(char* theText)
{
    ...
    if(SIM800_Commands[myCommandsCount].Command[0] != 0 || SIM800_Commands[myCommandsCount].Number[0] == 0)
        return false;
    
    if(strlen(theText) > 25)
        return false;
    
    memcpy(SIM800_Commands[myCommandsCount].Command, theText, strlen(theText));
}
